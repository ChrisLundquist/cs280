...
class OAException
{
  public:
    ...
    enum OA_EXCEPTION
    {
      ...
      E_BAD_ADDRESS,    // block address is within on a page
      E_BAD_BOUNDARY,   // block address is within a page, but not on any block-boundary
...
struct OAStats
{
  OAStats() : ObjectSize_(0), FreeObjects_(0), ObjectsInUse_(0), PagesInUse_(0),
...
class ObjectAllocator
{
  ...
  private:
    ...
    inline char* object_to_header(char*);
    inline char* object_to_left_pad(char*);
    inline char* object_to_right_pad(char*);
    inline char* allocation_to_object(char*);
    inline char* object_to_allocation(char*);
